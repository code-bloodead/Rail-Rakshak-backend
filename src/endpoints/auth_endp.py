...
@router.post("/save_staff_token")
def save_staff_token(staff : StaffLogin, response: Response):
    if staff.id == "" or staff.password == "" or staff.notifcation_token == "":
        response.status_code = status.HTTP_400_BAD_REQUEST
        return {"ERROR":"MISSING PARAMETERS"}
    ...
    if update_staff_token(staff.id, staff.notifcation_token) == "SUCCESS":
        return {"SUCCESS": "TOKEN SAVED"}
    else:
        response.status_code = status.HTTP_500_INTERNAL_SERVER_ERROR
        return {"ERROR":"Failed to Save"}